...
import { abortBuild, getRecentReport, getConsoleTexts, getGrafanaHost } from '../../api/jenkins';
...
class SwarmStatus extends Component {
  ...
  componentDidMount() {
    getGrafanaHost()
      .then(response => {
        this.setState({
          grafanaHost: response.data.REACT_APP_GRAFANA_HOST_URL
        });
      })
      .catch(err => {
        console.log(err);
          ...
          <FwTabs>
            ...
            <FwTabPanel name="Live Status">
              ...
              <div className={`${Styles.overflow} flex-item-border fw-m-8`}>
                  <iframe
                    title="Active Thread"
                    src={configs.setgrafanaurl(this.state.grafanaHost,
                      selectedProd,
                      Date.now(),
                      31
                    )}
              ...
              <div className={`${Styles.overflow} fw-card-1 fw-p-24 fw-flex`}>
                <iframe
                  ...
                  src={configs.setgrafanaurl(this.state.grafanaHost, selectedProd, Date.now(), 4)}
                  width="1000"
                  height="250"
                  frameBorder="5"
                />
